...
class Sound:
    ...
    def from_midi(fp, **kwargs):
        if isinstance(fp, str):
            ...
            raise TypeError(f"Type of argument 'fp', cannot be '{type(fp)}'.")
        path_sound_fonts = kwargs.get("path_sound_fonts", Units.PATH_MIDI_SOUND_FONTS)
        npath = mkstemp(suffix=".wav")[1]
        subprocess.check_output([Units.PATH_FLUID_SYNTH, "-ni", path_sound_fonts, path, "-F", npath])
        if is_temp:
            os.remove(path)
